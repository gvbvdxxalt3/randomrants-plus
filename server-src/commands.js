...
class CommandHandler {
			...
			function (args, userInfo, senderClient) {
				if (typeof args[0] == "string") {
					...
					if (commandName.length > 0) {
						if (_this.commandExists(commandName) && commandIsAllowed(senderClient, commandName)) {
							sendFeedbackLocal(
								senderClient,
				...
				var text = "";
				if (!userInfo.isOwner) {
					text += "[color css=orange][bold]To unlock all commands, get ownership powers![/bold][/color][br]";
				}
				text += "Command list: ";
	...
	searchUsersByKey(key, senderClient) {
		...
		for (var client of clients) {
			var lowercaseUsername = this.getUserInfo(client).username.toLowerCase().trim(); //trim(), just to make sure.
	...
	commandIsAllowed (client, commandName) {
		...
		if (userInfo.isOwner) { //Owners get all commands.
			return true;
		}
		
		if (!this.commandPermissions[commandName]) { //If the owner only flag on the command is false/empty, then return true.
			return true;
		}
