...
async function getUserProfilePictureResponse(username, req, res) {
	try {
		await storage.downloadFileResponseProxy(`user-${username}-profile`, {}, res, [
			"content-type",
		]);
	} catch (e) {
...
async function validateUser(username, password) {
	...
	if (password.length > cons.MAX_PASSWORD_LENGTH) {
		return {
			...
			error: true,
			message:
				"Password must be less than " + cons.MAX_PASSWORD_LENGTH + " characters.",
		};
	}
	if (username.indexOf(" ") > -1) {
		return {
	...
	if (username.length > cons.MAX_USERNAME_LENGTH) {
		return {
			...
			error: true,
			message:
				"Username must be less than " + cons.MAX_USERNAME_LENGTH + " characters.",
		};
	}
	var i = 0;
	while (i < username.length) {
                    ...
                    if (json.type == "postMessagePrivate") {
						if (typeof json.message == "string" && typeof json.targetUser == "string") {
                            ...
                            var messageJson = JSON.stringify({
							    type: "newMessage",
								message: "[color css=yellow]For [bold]"+targetUser+"[/bold]: [/color]"+json.message,
								username: ws._rrUsername,
								displayName: displayName,
								color: ws._rrUserColor,
							});
			...
			if (client._rrIsReady) {
				for (var client2 of wss.clients) {
					...
					if (client2._rrIsReady) {
						if (
							client._rrOtherCams[client2._rrConnectionID] !== client2._rrCameraCode
						) {
							client._rrOtherCams[client2._rrConnectionID] = client2._rrCameraCode;
							client.send(
								JSON.stringify({
						...
						if (client._rrOtherMics[client2._rrConnectionID] !== client2._rrMicCode) {
		...
		if (urlsplit[2] == "code" && req.method == "POST") {
			(async function () {
				try {
					...
					var roomBuffer = await storage.downloadFile(`room-${json.id}-info.json`);
	...
	if (urlsplit[1] == "account") {
		...
		if (urlsplit[2] == "signup" && req.method == "POST") {
			...
			req.on("end", async () => {
				...
				var stuff = await createUser(json.username.toLowerCase(), json.password);
				if (stuff.valid) {
			...
			(async function () {
				if (decryptedUserdata) {
					try {
						...
						if (
							decryptedUserdata.username.toLowerCase() == json.username.toLowerCase()
						) {
							res.statusCode = 400;
							res.end("");
							return;
		...
		if (urlsplit[2] == "login" && req.method == "POST") {
			...
			req.on("end", async () => {
				...
				var stuff = await validateUser(json.username.toLowerCase(), json.password);
				if (stuff.valid) {
					var value = encryptor.encrypt({
						username: json.username.toLowerCase().trim(),
						password: json.password,
